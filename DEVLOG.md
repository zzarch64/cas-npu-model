# CAS NPU Extension 开发日志

## 项目概述

本项目旨在为 PyTorch 提供 CAS NPU 后端支持，实现核心算子以支持深度学习模型（特别是 LLM）在 NPU 上的运行。

---

## 开发思路

### 核心策略：先跑通，再优化

采用**渐进式开发**策略，分为两个主要阶段：

```
┌─────────────────────────────────────────────────────────────────────────┐
│  阶段一：功能验证（Fallback 模式）                                        │
│  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━                                        │
│  目标：让模型完整跑起来，验证框架正确性                                    │
│                                                                         │
│  ┌──────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐          │
│  │ PyTorch  │───▶│ Device→  │───▶│ CPU 计算 │───▶│ →Device  │          │
│  │ 算子调用  │    │ CPU 拷贝 │    │ (现成实现)│    │ 拷贝回   │          │
│  └──────────┘    └──────────┘    └──────────┘    └──────────┘          │
│                                                                         │
│  ✓ 快速实现所有算子    ✓ 验证模型正确性    ✓ 建立测试基准               │
└─────────────────────────────────────────────────────────────────────────┘
                                   │
                                   ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  阶段二：性能优化（NPU 原生实现）                                         │
│  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━                                        │
│  目标：逐个替换为 NPU 原生算子，提升性能                                   │
│                                                                         │
│  ┌──────────┐    ┌─────────────────────────────────┐                   │
│  │ PyTorch  │───▶│ NPU Runtime API 直接执行        │                   │
│  │ 算子调用  │    │ (无 CPU 往返，最优性能)          │                   │
│  └──────────┘    └─────────────────────────────────┘                   │
│                                                                         │
│  ✓ 消除内存拷贝    ✓ 充分利用 NPU 算力    ✓ 达到生产性能                │
└─────────────────────────────────────────────────────────────────────────┘
```

### 为什么采用这种策略？

| 优势 | 说明 |
|-----|------|
| **降低开发风险** | 先用成熟的 CPU 实现验证整体框架，避免同时调试框架和算子 |
| **快速迭代** | Fallback 模式下可以快速支持新算子，让模型跑起来 |
| **明确优化方向** | 通过 profiling 确定性能瓶颈，按优先级优化最关键的算子 |
| **保持可测试性** | Fallback 实现作为参考，可用于验证 NPU 原生实现的正确性 |

### 具体迭代步骤

```
1. 新算子需求 ──▶ 2. 实现 Fallback 版本 ──▶ 3. 模型测试通过
                        │
                        ▼
4. 性能分析 ◀────────────┘
      │
      ▼
5. 高频算子优先实现 NPU 原生版本
      │
      ▼
6. 对比 Fallback 验证正确性 ──▶ 7. 替换为原生实现
```

### 当前状态

- ✅ **阶段一基本完成**：Qwen 0.5B 模型已能完整运行
- ⏳ **阶段二进行中**：已实现 `mm`、`bmm`、`add.Tensor` 的 NPU 原生版本
- 📋 **下一步**：按优先级实现 RMSNorm 相关算子（rsqrt, mul, pow, mean）

---

## Fallback 方案的性能代价分析

### 为什么 Fallback 方案性能差？

每次 `cpu_fallback` 执行都会产生大量的内存分配和释放操作：

```
┌─────────────────────────────────────────────────────────────┐
│  cpu_fallback 执行单个算子（如 rsqrt(x)）                     │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. 输入准备                                                │
│     ┌──────────────┐                                        │
│     │ CPU malloc   │ ← 为 x_cpu 分配 CPU 内存               │
│     └──────────────┘                                        │
│     ┌──────────────┐                                        │
│     │ casNpuMemcpy │ ← D→H 拷贝数据                         │
│     └──────────────┘                                        │
│                                                             │
│  2. CPU 计算                                                │
│     ┌──────────────┐                                        │
│     │ CPU malloc   │ ← 为 result_cpu 分配 CPU 内存          │
│     └──────────────┘                                        │
│     rsqrt(x_cpu) → result_cpu                               │
│                                                             │
│  3. 输出传回                                                │
│     ┌──────────────┐                                        │
│     │ casNpuMalloc │ ← 为 result_npu 分配 NPU 内存          │
│     └──────────────┘                                        │
│     ┌──────────────┐                                        │
│     │ casNpuMemcpy │ ← H→D 拷贝数据                         │
│     └──────────────┘                                        │
│                                                             │
│  4. 清理（函数返回后，引用计数归零时）                         │
│     ┌──────────────┐                                        │
│     │ CPU free     │ ← x_cpu、result_cpu 释放               │
│     │ casNpuFree   │ ← 旧 NPU tensor 释放（如果被覆盖）       │
│     └──────────────┘                                        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### 计算链的内存操作累积

以 `y = (x * 2 + 1).rsqrt()` 为例：

| 操作 | CPU malloc | CPU free | NPU malloc | NPU free |
|-----|-----------|----------|-----------|----------|
| `x * 2` | 2 | 2 | 1 | 0 |
| `... + 1` | 2 | 2 | 1 | 1* |
| `.rsqrt()` | 2 | 2 | 1 | 1* |
| **总计** | **6** | **6** | **3** | **2** |

*中间结果被下一步覆盖时释放

### 性能开销组成

```
┌─────────────────────────────────────────────────────┐
│  Fallback 开销组成                                   │
├─────────────────────────────────────────────────────┤
│  1. 数据传输延迟     │████████████████│ 最大开销     │
│  2. malloc/free 开销 │████████░░░░░░░░│ 显著开销     │
│  3. 内存碎片化       │████░░░░░░░░░░░░│ 累积影响     │
│  4. CPU 计算本身     │██░░░░░░░░░░░░░░│ 相对较小     │
└─────────────────────────────────────────────────────┘
```

### NPU 原生实现 vs Fallback 对比

| 方案 | malloc 次数 | free 次数 | memcpy 次数 |
|-----|------------|----------|------------|
| **NPU 原生** | 1 (仅输出) | 0 | 0 |
| **Fallback** | 2-3 | 2 | 2 (D→H + H→D) |

### 结论

1. **性能差距巨大**：Fallback 可能比原生实现慢 10-100 倍
2. **内存峰值高**：临时 tensor 会增加内存占用峰值
3. **GC 压力大**：频繁分配/释放给内存管理带来压力

**开发路线正确性**：先用 Fallback 跑通功能验证正确性，再逐步替换为 NPU 原生实现是合理的渐进策略。

---

## 架构设计

### 内存模型：显式 Copy 模式

采用类似 CUDA GPU 的独立内存架构，设备内存和 CPU 内存不共享，需要显式进行数据拷贝。

```
┌─────────────────────────────────────────────────────────────┐
│                      Host (CPU) Memory                       │
└───────────────────────────┬─────────────────────────────────┘
                            │
         casNpuMemcpy(HOST_TO_DEVICE) ↓ ↑ casNpuMemcpy(DEVICE_TO_HOST)
                            │
┌───────────────────────────┴─────────────────────────────────┐
│                    Device (NPU) Memory                       │
│                                                              │
│   casNpuMalloc() 分配    casNpuFree() 释放                   │
└─────────────────────────────────────────────────────────────┘
```

### 内存拷贝方向（类似 cudaMemcpyKind）

```cpp
enum CasNpuMemcpyKind {
    CAS_NPU_MEMCPY_HOST_TO_HOST = 0,      // CPU -> CPU
    CAS_NPU_MEMCPY_HOST_TO_DEVICE = 1,    // CPU -> Device
    CAS_NPU_MEMCPY_DEVICE_TO_HOST = 2,    // Device -> CPU
    CAS_NPU_MEMCPY_DEVICE_TO_DEVICE = 3,  // Device -> Device
    CAS_NPU_MEMCPY_DEFAULT = 4            // 自动检测
};
```

### 算子实现策略

| 实现方式 | 描述 | 性能 | 适用场景 |
|---------|------|------|---------|
| **NPU 原生实现** | 直接在 NPU 上执行，无内存拷贝 | ⭐⭐⭐ 最优 | 高频算子（mm, bmm, add） |
| **CPU Fallback** | 通过 `cpu_fallback` 自动处理 | ⭐ 较慢 | 开发阶段临时方案 |
| **View 操作** | 仅修改 metadata，无数据拷贝 | ⭐⭐⭐ 最优 | reshape, transpose, slice 等 |

### 代码架构

```
┌─────────────────────────────────────────────────────────────┐
│  PyTorch 算子层 (backend/cas_npu_ops.cpp)                    │
│  - NPU 原生实现：直接调用 Runtime API                         │
│  - CPU Fallback：通过 at::native::cpu_fallback 自动处理      │
│  - View 操作：仅修改 tensor metadata                         │
└───────────────────────┬─────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────────┐
│  NPU Runtime API (runtime/cas_npu_runtime.h)                │
│  - 内存管理：casNpuMalloc, casNpuFree, casNpuMemcpy         │
│  - 计算算子：casNpuMatMul, casNpuAddTensor, ...             │
└───────────────────────┬─────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────────┐
│  硬件实现层                                                  │
│  - runtime/cmodel/   : CPU 模拟实现（开发调试用）            │
│  - runtime/fpga/     : FPGA 硬件实现                        │
│  - runtime/asic/     : 未来 ASIC 芯片实现                   │
└─────────────────────────────────────────────────────────────┘
```

---

## Qwen 0.5B 推理算子进度表

### 📊 总体进度

| 类别 | NPU原生实现 | CPU Fallback | View操作 | 总计 |
|-----|------------|--------------|---------|------|
| 数量 | 3 | 17 | 13 | 33 |
| 状态 | ✅ 已完成 | ⚠️ 临时方案 | ✅ 已完成 | - |

### ✅ NPU 原生实现（高性能）

这些算子直接在 NPU 上执行，无需 CPU 往返，是性能关键路径。

| 算子 | Runtime API | 用途 | 状态 |
|-----|-------------|------|------|
| `mm` | `casNpuMatMul` | Linear 层、投影层 | ✅ 已实现 |
| `bmm` | `casNpuBatchMatMul` | Attention (Q@K^T, scores@V) | ✅ 已实现 |
| `add.Tensor` | `casNpuAddTensor` | 残差连接、偏置加法 | ✅ 已实现 |

### ⚠️ CPU Fallback 实现（需优化为 NPU 原生）

这些算子当前使用统一的 `cpu_fallback` 机制，PyTorch 自动处理 Device↔CPU 数据传输。

#### RMSNorm 相关（高优先级）
| 算子 | 待实现 Runtime API | 用途 | 调用频率 |
|-----|-------------------|------|---------|
| `rsqrt` | `casNpuRsqrt` | 1/sqrt(x)，RMSNorm 核心 | 每层 2 次 |
| `pow.Tensor_Scalar` | `casNpuPow` | x^2，计算方差 | 每层 2 次 |
| `mean.dim` | `casNpuMean` | 维度均值 | 每层 2 次 |

#### Rotary Embedding 相关（高优先级）
| 算子 | 待实现 Runtime API | 用途 | 调用频率 |
|-----|-------------------|------|---------|
| `cos` | `casNpuCos` | 位置编码 | 每层 1 次 |
| `sin` | `casNpuSin` | 位置编码 | 每层 1 次 |

#### 基础数学运算（中优先级）
| 算子 | 待实现 Runtime API | 用途 | 调用频率 |
|-----|-------------------|------|---------|
| `mul.Tensor` | `casNpuMulTensor` | 逐元素乘法 | 高 |
| `mul.Scalar` | `casNpuMulScalar` | 标量乘法 | 高 |
| `add.Scalar` | `casNpuAddScalar` | 标量加法 | 中 |
| `sub.Tensor` | `casNpuSubTensor` | 减法 | 低 |
| `div.Tensor` | `casNpuDivTensor` | 除法 | 低 |
| `neg` | `casNpuNeg` | 取负 | 低 |
| `sqrt` | `casNpuSqrt` | 平方根 | 低 |

#### 激活函数（高优先级）
| 算子 | 待实现 Runtime API | 用途 | 调用频率 |
|-----|-------------------|------|---------|
| `silu` | `casNpuSiLU` | SiLU 激活 (FFN) | 每层 1 次 |

#### Attention 相关（高优先级）
| 算子 | 待实现 Runtime API | 用途 | 调用频率 |
|-----|-------------------|------|---------|
| `softmax.int` | `casNpuSoftmax` | Attention 归一化 | 每层 1 次 |
| `scaled_dot_product_attention` | `casNpuSDPA` | 融合 Attention | 每层 1 次 |

#### 其他（低优先级）
| 算子 | 待实现 Runtime API | 用途 | 调用频率 |
|-----|-------------------|------|---------|
| `embedding` | `casNpuEmbedding` | Token 嵌入 | 仅输入层 |
| `cat` | `casNpuCat` | KV Cache 拼接 | 每层 2 次 |
| `clone` | `casNpuClone` | 张量复制 | 低 |
| `contiguous` | `casNpuContiguous` | 内存连续化 | 低 |

### ✅ View 操作（无需优化）

这些操作仅修改 tensor 的 metadata（shape, stride, offset），不涉及数据拷贝，已是最优实现。

| 算子 | 用途 | 状态 |
|-----|------|------|
| `view` | 改变形状 | ✅ 原生实现 |
| `reshape` | 改变形状 | ✅ 原生实现 |
| `transpose.int` | 交换维度 | ✅ 原生实现 |
| `permute` | 重排维度 | ✅ 原生实现 |
| `unsqueeze` | 插入维度 | ✅ 原生实现 |
| `squeeze` / `squeeze.dim` | 移除维度 | ✅ 原生实现 |
| `expand` | 广播扩展 | ✅ 原生实现 |
| `slice.Tensor` | 切片 | ✅ 原生实现 |
| `select.int` | 选择索引 | ✅ 原生实现 |
| `as_strided` | 自定义 stride | ✅ 原生实现 |
| `t` | 2D 转置 | ✅ 原生实现 |
| `detach` | 分离梯度 | ✅ 原生实现 |
| `_reshape_alias` | reshape 别名 | ✅ 原生实现 |

---

## 特殊算子处理说明

### `contiguous` - 保留自定义实现

`contiguous()` 保留了自定义实现而非使用 `cpu_fallback`，原因是可以优化"已经连续"的常见情况：

```cpp
at::Tensor cas_npu_contiguous(const at::Tensor& self, at::MemoryFormat memory_format) {
    // 如果已经是 contiguous 的，直接返回（零开销）
    if (self.is_contiguous(memory_format)) {
        return self;  // ← 无需任何拷贝！
    }
    
    // 真正需要连续化时，才走 fallback 路径
    at::Tensor self_cpu = device_to_cpu(self);
    at::Tensor contiguous_cpu = self_cpu.contiguous(memory_format);
    return cpu_to_device(contiguous_cpu, self.device());
}
```

| 实现方式 | 已连续时 | 非连续时 |
|---------|---------|---------|
| `cpu_fallback` | D→H→D（浪费） | D→H→D |
| 自定义实现 | 直接返回（零开销）✓ | D→H→D |

在深度学习模型中，90%+ 的 `contiguous()` 调用时 tensor 已经是连续的，自定义实现可避免大量无谓拷贝。

---

## 开发优先级

### 🔴 P0: 性能关键（每层多次调用）

**目标：消除最高频的 CPU 往返**

| 算子 | 预期性能提升 | 复杂度 |
|-----|-------------|-------|
| `rsqrt` | 高 | 低 |
| `mul.Tensor` / `mul.Scalar` | 高 | 低 |
| `silu` | 高 | 低 |
| `cos` / `sin` | 中 | 低 |
| `pow.Tensor_Scalar` | 中 | 低 |
| `mean.dim` | 中 | 中 |

### 🟡 P1: 重要优化

| 算子 | 预期性能提升 | 复杂度 |
|-----|-------------|-------|
| `softmax` | 高 | 中 |
| `scaled_dot_product_attention` | 很高（融合算子） | 高 |

### 🟢 P2: 完整性

| 算子 | 说明 |
|-----|------|
| `embedding` | 仅输入层调用 |
| `cat` | KV Cache 操作 |
| 其他基础算子 | 按需实现 |

---

## 开发计划

### Phase 1: 核心矩阵运算 ✅ 已完成
- [x] MM (矩阵乘法) - `casNpuMatMul`
- [x] BMM (批量矩阵乘法) - `casNpuBatchMatMul`
- [x] Add (张量加法) - `casNpuAddTensor`
- [x] 显式 Copy 内存模型 - `casNpuMemcpy` with direction

### Phase 2: RMSNorm 原生实现 ⏳ 进行中
- [ ] `casNpuRsqrt` - rsqrt
- [ ] `casNpuPow` - pow
- [ ] `casNpuMean` - mean.dim
- [ ] `casNpuMulTensor` / `casNpuMulScalar` - mul

### Phase 3: 激活函数 & 位置编码
- [ ] `casNpuSiLU` - silu 激活
- [ ] `casNpuCos` / `casNpuSin` - Rotary Embedding

### Phase 4: Attention 优化
- [ ] `casNpuSoftmax` - softmax
- [ ] `casNpuSDPA` - Scaled Dot-Product Attention（融合算子）

### Phase 5: 其他算子 & 性能优化
- [ ] Embedding
- [ ] 矩阵乘法性能优化（BLAS/专用库）
- [ ] Caching Allocator（可选优化）

### Phase 6: 🔴 LoRA Finetune 支持 🚧 待开发（最高优先级）
- [ ] 实现反向传播基础算子 (Fallback 方式)
- [ ] 验证 LeNet 训练正确性
- [ ] 实现 LoRA 层算子
- [ ] 验证 Qwen LoRA 微调端到端
- [ ] 优化关键反向算子为 NPU 原生实现

### Phase 7: CModel 物理内存抽象 🚧 待开发
- [ ] 设计 NPU 物理地址空间布局
- [ ] 实现物理地址分配器 (PhysicalMemoryAllocator)
- [ ] 实现 RAM 模拟器 (SimulatedRAM)
- [ ] 修改 casNpuMalloc/Free 返回物理地址
- [ ] 修改 casNpuMemcpy 支持物理地址访存
- [ ] 验证现有算子在物理内存模型下正确性

### Phase 8: RTL Model 后端 (Verilator) 🚧 待开发
- [ ] 设计 AXI 接口封装
- [ ] 定义命令包格式 (NpuCommandPacket)
- [ ] 实现 Verilator wrapper
- [ ] 实现 RAM 模型接口
- [ ] 集成 NPU RTL 代码
- [ ] 验证基础算子 (mm, add)

### Phase 9: 编译后端切换 🚧 待开发
- [ ] 重构 setup.py 支持 CAS_NPU_BACKEND 环境变量
- [ ] 添加 CMakeLists.txt（可选）
- [ ] 定义 Backend 抽象接口 (backend_interface.h)
- [ ] 重构 cmodel 为 CModelBackend
- [ ] 重构 fpga 为 FPGABackend
- [ ] 添加 rtlmodel 后端
- [ ] 添加 asic 后端框架

### Phase 10: Runtime 架构重构 🚧 待开发
- [ ] 定义 Backend 抽象接口
- [ ] 实现后端管理器
- [ ] 支持运行时后端切换

---

## 调试工具

### Debug 输出系统

通过环境变量控制调试输出级别：

```bash
# 级别 1：显示算子类型
CAS_NPU_DEBUG_LEVEL=1 python your_script.py

# 级别 2：显示算子类型 + 数据传输
CAS_NPU_DEBUG_LEVEL=2 python your_script.py

# 级别 3：显示算子类型 + 数据传输 + Runtime 详情
CAS_NPU_DEBUG_LEVEL=3 python your_script.py
```

输出格式：
- `[NPU]` (绿色) - NPU 原生实现
- `[CPU]` (红色) - CPU Fallback
- `[COPY]` (蓝色) - 数据传输 (H→D, D→H, D→D)
- `[VIEW]` (青色) - View 操作（无数据拷贝）

---

## 测试验证

### 当前测试状态

| 测试项 | 状态 | 说明 |
|-------|------|------|
| 基础 mm/bmm 测试 | ✅ 通过 | 精度误差 < 1e-5 |
| Linear 层测试 | ✅ 通过 | 768→3072 维度 |
| LeNet Forward | ✅ 通过 | CPU vs NPU 输出完全一致 |
| Qwen 0.5B Forward | ✅ 通过 | 完整前向传播 |

### 测试命令

```bash
# 运行 LeNet 测试
python test/test_lenet.py

# 运行 Qwen 模型测试
python test/test_qwen0.5B.py

# 运行自定义算子测试
python test/test_custom_ops.py

# 带调试输出运行
CAS_NPU_DEBUG_LEVEL=2 python test/test_lenet.py
```

---

## 相关文件

| 文件 | 说明 |
|-----|------|
| `runtime/cas_npu_runtime.h` | Runtime API 声明 |
| `runtime/cas_npu_debug.h` | 调试系统头文件 |
| `runtime/cmodel/simulator.cpp` | cmodel CPU 模拟实现 |
| `runtime/fpga/simulator.cpp` | FPGA 实现（待完善） |
| `backend/cas_npu_ops.cpp` | PyTorch 算子注册 |
| `backend/cas_npu_allocator.cpp` | 内存分配器 |
| `cas_npu/debug.py` | Python 调试接口 |
| `test/test_qwen0.5B.py` | Qwen 模型测试 |
| `test/test_lenet.py` | LeNet 模型测试 |

---

## 🔴 待开发：LoRA Finetune 支持（最高优先级）

### 需求背景

当前 CAS-NPU 仅支持推理（前向传播），要实现 LLM 的 LoRA 微调，需要支持：

1. **反向传播（Backward）**：计算梯度
2. **参数更新（Optimizer）**：AdamW 等优化器
3. **LoRA 层**：低秩适配器的高效计算

### 训练所需算子分析

#### 反向传播核心算子

| 前向算子 | 反向算子 | 说明 | 优先级 |
|---------|---------|------|-------|
| `mm` | `mm` (转置) | ∂L/∂A = ∂L/∂C @ B^T, ∂L/∂B = A^T @ ∂L/∂C | 🔴 P0 |
| `bmm` | `bmm` (转置) | 同上，批量版本 | 🔴 P0 |
| `add` | `add` (identity) | 梯度直接传递 | 🔴 P0 |
| `mul` | `mul` | ∂L/∂A = ∂L/∂C * B | 🔴 P0 |
| `softmax` | `softmax_backward` | Jacobian 计算 | 🟡 P1 |
| `silu` | `silu_backward` | σ(x) * (1 + x * (1 - σ(x))) | 🟡 P1 |
| `rms_norm` | `rms_norm_backward` | 复合梯度 | 🟡 P1 |

#### 优化器算子

| 算子 | 用途 | 说明 |
|-----|------|------|
| `addcmul` | AdamW | m = β1 * m + (1-β1) * grad |
| `addcdiv` | AdamW | param -= lr * m / (sqrt(v) + eps) |
| `mul_` | 权重衰减 | param *= (1 - lr * weight_decay) |
| `add_` | 参数更新 | param += update |

#### LoRA 特定算子

```
LoRA 计算流程：
                    ┌─────────────────────────────────────┐
                    │     原始权重 W [d_out, d_in]         │
                    │         (冻结)                       │
                    └─────────────────────────────────────┘
                                      +
┌─────────────────────────────────────────────────────────────────┐
│  LoRA 增量 ΔW = B @ A                                           │
│                                                                 │
│  x ──▶ [A: r×d_in] ──▶ [B: d_out×r] ──▶ scale ──▶ ΔW @ x       │
│        (可训练)        (可训练)         α/r                     │
│                                                                 │
│  关键算子：                                                      │
│  - mm: A @ x, B @ (A @ x)                                       │
│  - scale: 常数乘法                                               │
│  - add: W @ x + ΔW @ x                                          │
└─────────────────────────────────────────────────────────────────┘
```

### 实现路线

```
Phase 6.1: 基础训练支持
━━━━━━━━━━━━━━━━━━━━━━━━
├── 实现 backward 算子框架
├── mm_backward (最核心)
├── add_backward
└── 验证 LeNet 训练

Phase 6.2: LLM 训练算子
━━━━━━━━━━━━━━━━━━━━━━━━
├── softmax_backward
├── silu_backward
├── rms_norm_backward
└── 验证 Transformer block 训练

Phase 6.3: 优化器 & LoRA
━━━━━━━━━━━━━━━━━━━━━━━━
├── AdamW 优化器算子
├── LoRA 层高效实现
├── 混合精度支持 (可选)
└── Qwen LoRA 端到端验证

Phase 6.4: 性能优化
━━━━━━━━━━━━━━━━━━━━━━━━
├── 梯度累积
├── 梯度检查点
├── 算子融合 (Backward Fusion)
└── 内存优化
```

### 验证计划

| 阶段 | 测试项 | 验证内容 |
|-----|-------|---------|
| 6.1 | LeNet Training | 基础反向传播正确性 |
| 6.2 | Transformer Block | Attention + FFN 梯度 |
| 6.3 | Qwen LoRA | 端到端微调，loss 下降 |
| 6.4 | 性能测试 | 训练吞吐量对比 |

### 预期挑战

1. **内存管理**：训练需要保存激活值用于反向传播，内存需求增加
2. **数值精度**：梯度计算对精度敏感，需要验证 FP32 精度
3. **性能**：反向传播计算量约为前向的 2 倍

---

## 待开发：CModel 物理内存抽象

### 问题分析

当前 CModel 实现直接使用 CPU 的 `malloc`/`free` 和虚拟地址：

```cpp
// 当前实现 (runtime/cmodel/simulator.cpp)
CasNpuError casNpuMalloc(void** ptr, size_t size) {
    *ptr = malloc(size);  // ← 返回 CPU 虚拟地址
    return CAS_NPU_SUCCESS;
}

CasNpuError casNpuMemcpy(void* dst, const void* src, size_t size, CasNpuMemcpyKind kind) {
    memcpy(dst, src, size);  // ← 直接使用 CPU memcpy
    return CAS_NPU_SUCCESS;
}
```

**问题**：
1. 无法模拟真实 NPU 的物理内存行为
2. 与 RTL Model 和硬件的内存模型不一致
3. 无法验证地址对齐、内存布局等硬件约束

### 目标设计

#### 物理地址空间布局

```
NPU Physical Address Map (示例，可配置)
┌────────────────────────────────────────────────────────────┐
│  0x0000_0000 - 0x0FFF_FFFF  │  Weight Memory (256MB)       │
├────────────────────────────────────────────────────────────┤
│  0x1000_0000 - 0x1FFF_FFFF  │  Activation Memory (256MB)   │
├────────────────────────────────────────────────────────────┤
│  0x2000_0000 - 0x2000_FFFF  │  Register Space (64KB)       │
├────────────────────────────────────────────────────────────┤
│  0x2001_0000 - 0x3FFF_FFFF  │  Reserved                    │
└────────────────────────────────────────────────────────────┘
```

#### 核心组件设计

```cpp
// runtime/memory/physical_memory.h

namespace cas_npu {

// NPU 物理地址类型
using PhysAddr = uint64_t;

// 物理内存配置
struct MemoryConfig {
    PhysAddr weight_base = 0x00000000;
    size_t weight_size = 256 * 1024 * 1024;  // 256MB
    PhysAddr activation_base = 0x10000000;
    size_t activation_size = 256 * 1024 * 1024;  // 256MB
    size_t alignment = 64;  // 64字节对齐
};

// 模拟 RAM
class SimulatedRAM {
public:
    SimulatedRAM(size_t size);
    
    // 物理地址读写
    void write(PhysAddr addr, const void* data, size_t size);
    void read(PhysAddr addr, void* data, size_t size) const;
    
    // 获取物理地址对应的 host 指针（仅用于调试）
    void* getHostPtr(PhysAddr addr);
    
private:
    std::vector<uint8_t> memory_;
    PhysAddr base_addr_;
};

// 物理地址分配器
class PhysicalMemoryAllocator {
public:
    PhysicalMemoryAllocator(const MemoryConfig& config);
    
    // 分配/释放物理地址
    PhysAddr allocate(size_t size, size_t alignment = 0);
    void free(PhysAddr addr);
    
    // 查询
    size_t getAllocatedSize() const;
    size_t getFreeSize() const;
    
private:
    // 简单的 first-fit 分配器，后续可优化
    struct Block { PhysAddr addr; size_t size; bool free; };
    std::vector<Block> blocks_;
};

// 内存管理器（整合分配器和 RAM）
class MemoryManager {
public:
    static MemoryManager& instance();
    
    // 分配 NPU 内存，返回物理地址
    PhysAddr allocate(size_t size);
    void free(PhysAddr addr);
    
    // Host ↔ Device 数据传输
    void copyHostToDevice(PhysAddr dst, const void* src, size_t size);
    void copyDeviceToHost(void* dst, PhysAddr src, size_t size);
    void copyDeviceToDevice(PhysAddr dst, PhysAddr src, size_t size);
    
    // 获取 RAM 引用（供算子实现使用）
    SimulatedRAM& getRAM();
    
private:
    MemoryConfig config_;
    SimulatedRAM ram_;
    PhysicalMemoryAllocator allocator_;
};

} // namespace cas_npu
```

#### 修改后的 Runtime API 实现

```cpp
// runtime/cmodel/simulator.cpp (修改后)

CasNpuError casNpuMalloc(void** ptr, size_t size) {
    auto& mm = MemoryManager::instance();
    PhysAddr paddr = mm.allocate(size);
    if (paddr == 0) {
        return CAS_NPU_ERROR_OUT_OF_MEMORY;
    }
    // 返回物理地址（强制转换为指针形式，调用者需知道这是物理地址）
    *ptr = reinterpret_cast<void*>(paddr);
    return CAS_NPU_SUCCESS;
}

CasNpuError casNpuMemcpy(void* dst, const void* src, size_t size, CasNpuMemcpyKind kind) {
    auto& mm = MemoryManager::instance();
    
    switch (kind) {
        case CAS_NPU_MEMCPY_HOST_TO_DEVICE:
            mm.copyHostToDevice(reinterpret_cast<PhysAddr>(dst), src, size);
            break;
        case CAS_NPU_MEMCPY_DEVICE_TO_HOST:
            mm.copyDeviceToHost(const_cast<void*>(dst), 
                               reinterpret_cast<PhysAddr>(src), size);
            break;
        case CAS_NPU_MEMCPY_DEVICE_TO_DEVICE:
            mm.copyDeviceToDevice(reinterpret_cast<PhysAddr>(dst),
                                  reinterpret_cast<PhysAddr>(src), size);
            break;
        // ...
    }
    return CAS_NPU_SUCCESS;
}

// 算子实现需要修改为使用物理地址访存
CasNpuError casNpuAddTensor(float* output, const float* input1,
                            const float* input2, size_t num_elements, float alpha) {
    auto& ram = MemoryManager::instance().getRAM();
    PhysAddr out_addr = reinterpret_cast<PhysAddr>(output);
    PhysAddr in1_addr = reinterpret_cast<PhysAddr>(input1);
    PhysAddr in2_addr = reinterpret_cast<PhysAddr>(input2);
    
    // 通过物理地址访问 RAM
    for (size_t i = 0; i < num_elements; ++i) {
        float v1, v2, result;
        ram.read(in1_addr + i * sizeof(float), &v1, sizeof(float));
        ram.read(in2_addr + i * sizeof(float), &v2, sizeof(float));
        result = v1 + alpha * v2;
        ram.write(out_addr + i * sizeof(float), &result, sizeof(float));
    }
    return CAS_NPU_SUCCESS;
}
```

### 迁移计划

1. **Phase 1**：实现 `SimulatedRAM` 和 `PhysicalMemoryAllocator`
2. **Phase 2**：实现 `MemoryManager`，修改 `casNpuMalloc`/`casNpuFree`
3. **Phase 3**：修改 `casNpuMemcpy` 支持物理地址
4. **Phase 4**：逐个修改算子实现使用物理地址访存
5. **Phase 5**：验证所有现有测试通过

---

## 待开发：RTL Model 后端 (Verilator)

### 目标

使用 Verilator 将 NPU IP 的 Verilog/SystemVerilog RTL 代码编译为 C++ 仿真模型，作为一个后端实现。

### 架构设计

```
┌─────────────────────────────────────────────────────────────────┐
│  PyTorch Backend Layer                                          │
│  casNpuMatMul(), casNpuAddTensor(), ...                        │
├─────────────────────────────────────────────────────────────────┤
│  RTL Model Backend (runtime/rtlmodel/)                          │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  RTLModelBackend : CasNpuBackend                         │   │
│  │  - 实现所有 Runtime API                                   │   │
│  │  - 将 API 调用转换为命令包                                 │   │
│  └─────────────────────────────────────────────────────────┘   │
│                           │                                     │
│                           ▼                                     │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  VerilatorWrapper                                        │   │
│  │  - 管理仿真时钟                                           │   │
│  │  - 驱动 AXI 接口                                         │   │
│  │  - 处理中断/完成信号                                      │   │
│  └─────────────────────────────────────────────────────────┘   │
│                           │                                     │
│                           ▼                                     │
├─────────────────────────────────────────────────────────────────┤
│  Verilator Generated (obj_dir/)                                 │
│  ├─ Vnpu_top.h           - 顶层模块头文件                       │
│  ├─ Vnpu_top.cpp         - 仿真实现                            │
│  └─ Vnpu_top__ALL.a      - 静态库                              │
├─────────────────────────────────────────────────────────────────┤
│  NPU RTL (rtl/)                                                 │
│  ├─ npu_top.v            - 顶层 (AXI Slave + Control)          │
│  ├─ axi_slave.v          - AXI4 Slave 接口                     │
│  ├─ matrix_engine.v      - 矩阵计算引擎                         │
│  ├─ memory_controller.v  - 内存控制器                           │
│  └─ ...                                                         │
└─────────────────────────────────────────────────────────────────┘
```

### 命令包设计

```cpp
// runtime/rtlmodel/command_packet.h

namespace cas_npu {

// 操作码定义
enum class NpuOpcode : uint32_t {
    NOP         = 0x00,
    MATMUL      = 0x01,  // 矩阵乘法
    ADD_TENSOR  = 0x02,  // 张量加法
    MEMCPY      = 0x03,  // 内存拷贝
    SYNC        = 0xFF,  // 同步
};

// 命令包结构（对应 RTL 接口）
struct alignas(64) NpuCommandPacket {
    uint32_t opcode;           // [31:0]  操作码
    uint32_t flags;            // [63:32] 标志位
    uint64_t src1_addr;        // 源地址1 (物理地址)
    uint64_t src2_addr;        // 源地址2 (物理地址)
    uint64_t dst_addr;         // 目标地址 (物理地址)
    uint32_t param[8];         // 操作参数
    // param[0-1]: M (int64)
    // param[2-3]: K (int64)
    // param[4-5]: N (int64)
    // param[6]:   alpha (float, reinterpret)
    // param[7]:   reserved
};

// 状态寄存器
struct NpuStatus {
    uint32_t state;      // 0=IDLE, 1=BUSY, 2=DONE, 3=ERROR
    uint32_t error_code;
    uint64_t cycle_count;
};

} // namespace cas_npu
```

### AXI 驱动实现

```cpp
// runtime/rtlmodel/axi_driver.h

namespace cas_npu {

class AxiDriver {
public:
    AxiDriver(Vnpu_top* dut);
    
    // AXI4-Lite 寄存器访问
    void writeReg(uint32_t addr, uint32_t data);
    uint32_t readReg(uint32_t addr);
    
    // AXI4 内存访问
    void writeMem(uint64_t addr, const void* data, size_t size);
    void readMem(uint64_t addr, void* data, size_t size);
    
    // 命令接口
    void submitCommand(const NpuCommandPacket& cmd);
    void waitComplete(uint32_t timeout_cycles = 1000000);
    NpuStatus getStatus();
    
private:
    Vnpu_top* dut_;
    
    // AXI 握手
    void axiWriteTransaction(uint64_t addr, const uint8_t* data, size_t len);
    void axiReadTransaction(uint64_t addr, uint8_t* data, size_t len);
    
    // 时钟驱动
    void tick();
};

} // namespace cas_npu
```

### Verilator Wrapper

```cpp
// runtime/rtlmodel/verilator_wrapper.h

namespace cas_npu {

class VerilatorWrapper {
public:
    VerilatorWrapper();
    ~VerilatorWrapper();
    
    // 初始化/复位
    void reset(int cycles = 10);
    
    // 时钟控制
    void tick();
    void run(uint64_t cycles);
    
    // 获取 DUT 和驱动
    Vnpu_top* getDut() { return dut_.get(); }
    AxiDriver& getAxiDriver() { return *axi_driver_; }
    
    // RAM 模型
    SimulatedRAM& getRAM() { return ram_; }
    
    // 统计
    uint64_t getCycleCount() const { return cycle_count_; }
    
private:
    std::unique_ptr<Vnpu_top> dut_;
    std::unique_ptr<VerilatedVcdC> trace_;  // 波形追踪（可选）
    std::unique_ptr<AxiDriver> axi_driver_;
    SimulatedRAM ram_;
    uint64_t cycle_count_ = 0;
};

} // namespace cas_npu
```

### RTL Model 后端实现

```cpp
// runtime/rtlmodel/backend.cpp

class RTLModelBackend : public CasNpuBackend {
public:
    const char* name() const override { return "rtlmodel"; }
    
    CasNpuError matMul(float* output, const float* input1, const float* input2,
                       int64_t M, int64_t K, int64_t N) override {
        // 1. 构造命令包
        NpuCommandPacket cmd = {};
        cmd.opcode = static_cast<uint32_t>(NpuOpcode::MATMUL);
        cmd.src1_addr = reinterpret_cast<uint64_t>(input1);
        cmd.src2_addr = reinterpret_cast<uint64_t>(input2);
        cmd.dst_addr = reinterpret_cast<uint64_t>(output);
        memcpy(&cmd.param[0], &M, sizeof(int64_t));
        memcpy(&cmd.param[2], &K, sizeof(int64_t));
        memcpy(&cmd.param[4], &N, sizeof(int64_t));
        
        // 2. 提交命令
        wrapper_.getAxiDriver().submitCommand(cmd);
        
        // 3. 等待完成
        wrapper_.getAxiDriver().waitComplete();
        
        // 4. 检查状态
        auto status = wrapper_.getAxiDriver().getStatus();
        if (status.state == 3) {  // ERROR
            return CAS_NPU_ERROR_UNKNOWN;
        }
        
        return CAS_NPU_SUCCESS;
    }
    
    // ... 其他算子实现类似
    
private:
    VerilatorWrapper wrapper_;
};
```

### 构建流程

```bash
# 1. 编译 RTL 为 Verilator 模型
verilator --cc --exe --build \
    -I rtl/ \
    rtl/npu_top.v \
    -o obj_dir/Vnpu_top

# 2. 构建 Python 扩展（包含 RTL Model）
CAS_NPU_BACKEND=rtlmodel python setup.py build_ext --inplace
```

### 目录结构（新增）

```
runtime/
├── rtlmodel/
│   ├── backend.cpp           # RTLModelBackend 实现
│   ├── backend.h
│   ├── verilator_wrapper.cpp # Verilator 封装
│   ├── verilator_wrapper.h
│   ├── axi_driver.cpp        # AXI 总线驱动
│   ├── axi_driver.h
│   ├── command_packet.h      # 命令包定义
│   └── Makefile.verilator    # Verilator 构建脚本
├── memory/
│   ├── physical_memory.cpp   # 物理内存模型
│   ├── physical_memory.h
│   └── simulated_ram.cpp     # 模拟 RAM
└── ...
```

---

## 待开发：编译后端切换

### 目标

通过环境变量 `CAS_NPU_BACKEND` 在编译时选择不同的后端实现。

### setup.py 改进

```python
# setup.py

import os
from setuptools import setup
from torch.utils.cpp_extension import BuildExtension, CppExtension

# 读取后端选择，默认 cmodel
backend = os.environ.get('CAS_NPU_BACKEND', 'cmodel')
valid_backends = ['cmodel', 'rtlmodel', 'fpga', 'asic']

if backend not in valid_backends:
    print(f"Warning: Invalid CAS_NPU_BACKEND='{backend}', using 'cmodel'")
    backend = 'cmodel'

print(f"Building CAS-NPU with backend: {backend}")

# 通用源文件
common_sources = [
    'backend/cas_npu_allocator.cpp',
    'backend/cas_npu_guard.cpp',
    'backend/cas_npu_hooks.cpp',
    'backend/cas_npu_ops.cpp',
    'backend/cas_npu_module.cpp',
    'backend/cas_npu_custom_ops_example.cpp',
    'runtime/memory/physical_memory.cpp',  # 新增：物理内存模型
]

# 后端特定源文件
backend_sources = {
    'cmodel': [
        'runtime/cmodel/backend.cpp',
    ],
    'rtlmodel': [
        'runtime/rtlmodel/backend.cpp',
        'runtime/rtlmodel/verilator_wrapper.cpp',
        'runtime/rtlmodel/axi_driver.cpp',
    ],
    'fpga': [
        'runtime/fpga/backend.cpp',
    ],
    'asic': [
        'runtime/asic/backend.cpp',
    ],
}

# 后端特定编译选项
backend_compile_args = {
    'cmodel': [],
    'rtlmodel': ['-I/path/to/verilator/include'],  # Verilator 头文件
    'fpga': [],
    'asic': [],
}

# 后端特定链接选项
backend_link_args = {
    'cmodel': [],
    'rtlmodel': ['-L./obj_dir', '-lVnpu_top'],  # Verilator 生成的库
    'fpga': [],
    'asic': [],
}

# 后端特定宏定义
backend_define_macros = {
    'cmodel':   [('CAS_NPU_BACKEND_CMODEL', '1')],
    'rtlmodel': [('CAS_NPU_BACKEND_RTLMODEL', '1')],
    'fpga':     [('CAS_NPU_BACKEND_FPGA', '1')],
    'asic':     [('CAS_NPU_BACKEND_ASIC', '1')],
}

sources = common_sources + backend_sources[backend]

extra_compile_args = {
    'cxx': ['-std=c++17', '-O3', '-Wall'] + backend_compile_args[backend],
}

setup(
    name='cas_npu',
    version='0.1.0',
    ext_modules=[
        CppExtension(
            name='cas_npu._cas_npu_C',
            sources=sources,
            include_dirs=['.', 'backend', 'runtime'],
            extra_compile_args=extra_compile_args,
            extra_link_args=backend_link_args[backend],
            define_macros=backend_define_macros[backend],
        ),
    ],
    cmdclass={'build_ext': BuildExtension},
)
```

### 编译命令汇总

```bash
# CModel 后端（默认）
python setup.py build_ext --inplace

# 或显式指定
CAS_NPU_BACKEND=cmodel python setup.py build_ext --inplace

# RTL Model 后端
# 注意：需要先用 Verilator 编译 RTL
cd runtime/rtlmodel && make -f Makefile.verilator && cd ../..
CAS_NPU_BACKEND=rtlmodel python setup.py build_ext --inplace

# FPGA 后端
CAS_NPU_BACKEND=fpga python setup.py build_ext --inplace

# ASIC 后端
CAS_NPU_BACKEND=asic python setup.py build_ext --inplace
```

### 运行时后端查询

```python
import cas_npu

# 查询当前后端
print(cas_npu.get_backend_name())  # "cmodel" / "rtlmodel" / ...

# 查询后端信息
print(cas_npu.get_backend_info())
# {
#     "name": "rtlmodel",
#     "version": "0.1.0",
#     "features": ["matmul", "add", "bmm"],
# }
```

---

## 待开发：Runtime 架构重构

### 问题分析

当前 Runtime 架构存在以下问题：

```
当前架构（问题）：
┌─────────────────────────────────────────────────────────────────┐
│  runtime/cas_npu_runtime.h                                      │
│  - 所有 API 声明 (casNpuMalloc, casNpuMatMul, ...)              │
│  - 没有抽象层，直接声明具体函数                                    │
└───────────────────────┬─────────────────────────────────────────┘
                        │ 编译时选择一个
         ┌──────────────┼──────────────┐
         ▼              ▼              ▼
┌────────────────┐ ┌────────────────┐ ┌────────────────┐
│ cmodel/        │ │ fpga/          │ │ asic/          │
│ simulator.cpp  │ │ simulator.cpp  │ │ (待实现)        │
│ 直接实现所有函数 │ │ 直接实现所有函数 │ │                │
└────────────────┘ └────────────────┘ └────────────────┘

问题：
1. 编译时绑定，无法运行时切换后端
2. 每个后端需要完整实现所有函数，代码重复
3. 添加新后端需要修改多处代码
4. 测试困难，无法 mock 后端
```

### 目标架构

```
目标架构（解耦）：
┌─────────────────────────────────────────────────────────────────┐
│  runtime/cas_npu_runtime.h                                      │
│  - 对外统一接口（不变）                                           │
│  - 内部委托给当前后端                                             │
├─────────────────────────────────────────────────────────────────┤
│  runtime/backend_interface.h  [NEW]                             │
│  - CasNpuBackend 抽象基类                                        │
│  - 纯虚函数定义所有后端需要实现的接口                               │
├─────────────────────────────────────────────────────────────────┤
│  runtime/backend_manager.h  [NEW]                               │
│  - 后端注册表                                                    │
│  - 运行时后端选择 (getBackend(), setBackend())                   │
│  - 默认后端配置                                                  │
└───────────────────────┬─────────────────────────────────────────┘
                        │ 运行时选择
         ┌──────────────┼──────────────┐
         ▼              ▼              ▼
┌────────────────┐ ┌────────────────┐ ┌────────────────┐
│ cmodel/        │ │ fpga/          │ │ asic/          │
│ backend.cpp    │ │ backend.cpp    │ │ backend.cpp    │
│ CModelBackend  │ │ FPGABackend    │ │ ASICBackend    │
│ : CasNpuBackend│ │ : CasNpuBackend│ │ : CasNpuBackend│
└────────────────┘ └────────────────┘ └────────────────┘
```

### 接口设计草案

```cpp
// runtime/backend_interface.h
namespace cas_npu {

class CasNpuBackend {
public:
    virtual ~CasNpuBackend() = default;
    
    // 后端信息
    virtual const char* name() const = 0;
    virtual const char* version() const = 0;
    
    // 设备管理
    virtual CasNpuError getDeviceCount(int* count) = 0;
    virtual CasNpuError setDevice(int device) = 0;
    virtual CasNpuError getDevice(int* device) = 0;
    
    // 内存管理
    virtual CasNpuError malloc(void** ptr, size_t size) = 0;
    virtual CasNpuError free(void* ptr) = 0;
    virtual CasNpuError memcpy(void* dst, const void* src, 
                               size_t size, CasNpuMemcpyKind kind) = 0;
    virtual CasNpuError memset(void* ptr, int value, size_t size) = 0;
    
    // 计算算子
    virtual CasNpuError addTensor(float* output, const float* input1,
                                  const float* input2, size_t num_elements,
                                  float alpha = 1.0f) = 0;
    virtual CasNpuError matMul(float* output, const float* input1,
                               const float* input2,
                               int64_t M, int64_t K, int64_t N) = 0;
    virtual CasNpuError batchMatMul(float* output, const float* input1,
                                    const float* input2,
                                    int64_t B, int64_t M, int64_t K, int64_t N) = 0;
    // ... 更多算子
};

// 后端管理器
class BackendManager {
public:
    static BackendManager& instance();
    
    void registerBackend(const std::string& name, 
                        std::unique_ptr<CasNpuBackend> backend);
    CasNpuBackend* getBackend(const std::string& name = "");
    void setDefaultBackend(const std::string& name);
    std::vector<std::string> listBackends() const;
    
private:
    std::unordered_map<std::string, std::unique_ptr<CasNpuBackend>> backends_;
    std::string defaultBackend_ = "cmodel";
};

} // namespace cas_npu
```

### 迁移计划

1. **Phase 1**：创建接口定义（不破坏现有代码）
2. **Phase 2**：将 cmodel 实现适配为 CModelBackend
3. **Phase 3**：将 fpga 实现适配为 FPGABackend
4. **Phase 4**：修改 `cas_npu_runtime.h` 使用后端管理器
5. **Phase 5**：添加 Python 绑定支持后端切换
6. **Phase 6**：创建 asic 后端框架

---

## 备注

1. **CPU Fallback 是临时方案**：当前大部分算子使用 `cpu_fallback` 机制，这是为了快速验证功能正确性，不是最终方案。

2. **性能优化的关键**：在 NPU Runtime 层原生实现算子，消除 CPU 往返和频繁的 malloc/free 开销。

3. **内存效率**：当 NPU 原生算子完善后，可考虑实现 Caching Allocator 进一步优化内存分配。

4. **多后端支持**：Runtime 层支持 cmodel（调试）和 fpga/asic（生产）两种实现，通过编译选项切换。

5. **LoRA Finetune 优先**：当前最高优先级是实现 LoRA Finetune 支持，以验证训练功能。架构重构和其他优化可以在此之后进行。
