    ...
    if show_progress:
        try:
            ...
            if generation_thread.is_alive():
                print("\n  ⚠ Generation thread still running, may be stuck")
            ...
            # 如果 streamer 不可用或失败，回退到普通生成
            print(f"\n  ⚠ Streamer failed: {streamer_error}")
            print("  Falling back to normal generation...")
            # 继续执行普通生成逻辑
    
    # 普通生成（不使用进度显示，或 streamer 失败后的回退）
    if not show_progress or 'streamer_error' in locals():
        with torch.no_grad():
            try:
